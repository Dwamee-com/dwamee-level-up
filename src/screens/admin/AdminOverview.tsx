...
export default function AdminOverview() {
  ...
  const shieldDistribution = [
    { name: 'None', value: allEmployees.filter(e => e.shieldType === 'none').length, color: 'hsl(215 12% 50%)' },
    { name: 'Silver', value: allEmployees.filter(e => e.shieldType === 'silver').length, color: 'hsl(210 10% 65%)' },
    { name: 'Gold', value: allEmployees.filter(e => e.shieldType === 'gold').length, color: 'hsl(43 90% 55%)' },
    { name: 'Platinum', value: allEmployees.filter(e => e.shieldType === 'platinum').length, color: 'hsl(260 40% 60%)' },
  ].filter(d => d.value > 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: DollarSign, label: 'Total Salaries (YTD)', value: `$${totalSalaries.toLocaleString()}`, color: 'text-success', bg: 'bg-emerald-50' },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={deptData}>
                ...
                <Bar dataKey="avgLevel" name="Avg Level" fill="hsl(207 75% 40%)" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="glass-card p-5">
          ...
          <div className="h-56 flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={shieldDistribution} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} innerRadius={40} strokeWidth={2} stroke="hsl(0 0% 100%)">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="glass-card p-5">
          ...
          <div className="space-y-3">
            {[...allEmployees].sort((a, b) => b.xp - a.xp).slice(0, 5).map((emp, i) => (
              <div key={emp.id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-secondary/50 transition-colors">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="glass-card p-5">
          ...
          <div className="space-y-3">
            {monthlyWinners.map((w) => (
              <div key={`${w.month}-${w.year}`} className="flex items-center gap-3 p-2 rounded-xl hover:bg-secondary/50 transition-colors">
